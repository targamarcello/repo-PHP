...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { //variabile superglobal, fatte per essere chiamate dappertutto
    ...
    if (empty($nome) || empty($cognome) || empty($password) || empty($iscrizione)) {
        ...
        if ($row['max_tessera'] == null) {
            $max_tessera = 1;
        } else {
            $max_tessera = $row['max_tessera'] + 1;
        }
        $hash = password_hash($password, PASSWORD_DEFAULT);//password hashata


        //inserimento utente nuovo
        $queryI = 'INSERT INTO utenti(numero_tessera, nome, cognome, data_iscrizione, password) VALUES (?, ?, ?, ?, ?)';
        $stmt = $db->prepare($queryI);

        if ($stmt->execute([$max_tessera, $nome, $cognome, $iscrizione, $hash])) {
